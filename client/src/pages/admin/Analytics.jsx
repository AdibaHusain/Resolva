...
const STATUS_COLORS = {
  open: '#F59E0B', assigned: '#00E5FF',
  in_progress: '#6366F1', resolved: '#0EA5E9',
  verified: '#0EA5E9', rejected: '#EF4444',
}
...
const CustomTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div style={{
      ...
      {label && <div style={{ color: '#64748B', marginBottom: '6px', fontSize: '11px' }}>{label}</div>}
      {payload.map((p, i) => (
...
export default function Analytics() {
  ...
  const fetchAnalytics = async () => {
    ...
    try {
      ...
      const priorityData = [
        ...
        { name: 'Medium',   value: complaints.filter(c => c.priority === 'medium').length,   color: '#0EA5E9' },
        { name: 'Low',      value: complaints.filter(c => c.priority === 'low').length,      color: '#475569' },
      ]
  ...
  if (loading) return (
    <Layout>
      <div style={{ padding: '28px 32px', background: '#060B14', minHeight: '100vh' }}>
        <div style={{ marginBottom: '24px' }}>
          <div style={{ height: 28, width: 160, borderRadius: 8, background: '#080E1A', marginBottom: 8, animation: 'sk 1.4s ease-in-out infinite' }}/>
          <div style={{ height: 16, width: 240, borderRadius: 6, background: '#080E1A', animation: 'sk 1.4s ease-in-out infinite' }}/>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12, marginBottom: 20 }}>
          {Array.from({length:4}).map((_,i) => <div key={i} style={{height:100,borderRadius:12,background:'#080E1A',border:'1px solid rgba(0,229,255,0.06)',animation:'sk 1.4s ease-in-out infinite'}}/>)}
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          {Array.from({length:4}).map((_,i) => <div key={i} style={{height:280,borderRadius:14,background:'#080E1A',border:'1px solid rgba(0,229,255,0.06)',animation:'sk 1.4s ease-in-out infinite'}}/>)}
        </div>
  ...
  return (
    <Layout>
      <style>{`
        ...
        .an { padding: 28px 32px; min-height: 100vh; background: #060B14; }
        ...
        .an-title { font-size: 20px; font-weight: 600; color: #F8FAFC; letter-spacing: -0.3px; }
        .an-sub   { font-size: 13px; color: #64748B; margin-top: 2px; }
        .an-badge {
          display: flex; align-items: center; gap: 6px;
          padding: 6px 12px; border-radius: 8px;
          background: rgba(0,229,255,0.06);
          border: 1px solid rgba(0,229,255,0.18);
          font-size: 11.5px; font-weight: 500; color: #00E5FF;
        }
        .an-live-dot {
          width: 6px; height: 6px; border-radius: 50%;
          background: #00E5FF; animation: pulse 2s ease-in-out infinite;
        }
        ...
        .kpi:hover { border-color: rgba(0,229,255,0.2); transform: translateY(-2px); box-shadow: 0 8px 30px rgba(0,0,0,0.3); }
        ...
        .chart-card:hover { border-color: rgba(0,229,255,0.15); }

        .cc-head { display: flex; align-items: flex-start; justify-content: space-between; margin-bottom: 16px; }
        .cc-title { font-size: 13px; font-weight: 600; color: #F1F5F9; margin-bottom: 2px; }
        .cc-sub   { font-size: 11px; color: #475569; }
        .cc-badge {
        ...
        .cat-name { font-size: 12px; color: #E2E8F0; min-width: 110px; font-weight: 400; }
        .cat-bar-track {
        ...
        .cat-val { font-size: 11px; color: #475569; min-width: 24px; text-align: right; font-weight: 500; }
        ...
        .rec-table th {
          ...
          padding: 0 0 10px; border-bottom: 1px solid rgba(0,229,255,0.08);
        }
        .rec-table td {
          padding: 10px 0; border-bottom: 1px solid rgba(0,229,255,0.05);
          font-size: 12px; vertical-align: middle;
        }
        .rec-table tr:last-child td { border-bottom: none; }
        .rec-title {
          color: #F1F5F9; font-weight: 500; max-width: 180px;
        ...
        .dl-name { font-size: 12px; color: #64748B; }
        .dl-val  { font-size: 12px; font-weight: 600; color: #F1F5F9; }
      ...
      <div className="an">
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <button
              ...
              style={{
                ...
                border: '1px solid rgba(0,229,255,0.1)', color: '#64748B',
              ...
              onMouseLeave={e => { e.currentTarget.style.color='#64748B'; e.currentTarget.style.borderColor='rgba(0,229,255,0.1)' }}
            >
        ...
        <div className="kpi-row">
          {[
            {
              label: 'Total Complaints', num: data.total, color: '#00E5FF',
              trend: '+12%', trendUp: true,
              icon: <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#00E5FF" strokeWidth="1.6" strokeLinecap="round"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
            },
            {
              label: 'Resolved', num: data.resolved, color: '#0EA5E9',
              trend: `${data.resRate}% rate`, trendUp: true,
              icon: <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#0EA5E9" strokeWidth="1.6" strokeLinecap="round"><polyline points="20 6 9 17 4 12"/></svg>
            },
            {
            ...
            <div className="kpi" key={i}>
              <div className="kpi-top">
                ...
                <div className="kpi-trend" style={{
                  color: k.trendUp ? '#00E5FF' : '#F59E0B',
                  background: k.trendUp ? 'rgba(0,229,255,0.08)' : 'rgba(245,158,11,0.1)',
                  border: `1px solid ${k.trendUp ? 'rgba(0,229,255,0.2)' : 'rgba(245,158,11,0.2)'}`,
                }}>
                  {k.trend}
                </div>
              </div>
              ...
              <div className="kpi-bar" style={{ background: `linear-gradient(90deg, ${k.color}30, transparent)` }}/>
            </div>
          ))}
        </div>
          ...
          <div className="chart-card">
            ...
            <ResponsiveContainer width="100%" height={200}>
              <AreaChart data={data.dailyData}>
                ...
                <Area type="monotone" dataKey="submitted" name="Submitted" stroke="#00E5FF" strokeWidth={2} fill="url(#gS)" dot={false}/>
                <Area type="monotone" dataKey="resolved"  name="Resolved"  stroke="#6366F1" strokeWidth={2} fill="url(#gR)" dot={false}/>
              </AreaChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="chart-card">
            ...
            <div className="cat-list">
              {data.categoryData.slice(0, 7).map((cat, i) => {
                ...
                const colors = ['#00E5FF','#0EA5E9','#6366F1','#F59E0B','#EF4444','#22D3EE','#818CF8']
                return (
        ...
        {/* Row 2 — Severity bar + Priority donut */}
              ...
              <BarChart data={data.severityData} barSize={18}>
                ...
                <Bar dataKey="avg" name="Avg Score" radius={[4,4,0,0]}>
                  {data.severityData.map((entry, i) => {
                    const color = entry.avg >= 8 ? '#EF4444' : entry.avg >= 6 ? '#F59E0B' : entry.avg >= 4 ? '#0EA5E9' : '#00E5FF'
        ...
        <div className="full-row">
          <div className="chart-card">
            ...
            <table className="rec-table">
              ...
              <tbody>
                {data.recent.map(c => {
                  const prColor = { critical:'#EF4444', high:'#F59E0B', medium:'#0EA5E9', low:'#475569' }[c.priority] || '#475569'
                  const stColor = STATUS_COLORS[c.status] || '#64748B'
                  return (
                    <tr key={c._id}>
                      <td><div className="rec-title">{c.title}</div></td>
                      <td>
                      ...
                      <td>
                        <span className="rec-badge" style={{ color: stColor, background: `${stColor}14`, borderColor: `${stColor}35` }}>
                          {c.status.replace('_', ' ')}
                        </span>
                      </td>
                      <td>
                        <span className="rec-badge" style={{ color: prColor, background: `${prColor}14`, borderColor: `${prColor}35` }}>
                          {c.priority}
                        </span>
                      </td>
                      <td style={{ textAlign: 'right' }}>
                        <span style={{
                          ...
                          color: c.severityScore >= 8 ? '#EF4444' : c.severityScore >= 6 ? '#F59E0B' : '#00E5FF',
                        }}>
                          {c.severityScore}/10
                        </span>
                      </td>
